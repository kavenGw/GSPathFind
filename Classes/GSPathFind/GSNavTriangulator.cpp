...
void TriangulatorPoly::Init() {
    Clear();
}

void TriangulatorPoly::Triangle(GSNavPoint &p1, GSNavPoint &p2, GSNavPoint &p3) {
    points.clear();
    points.push_back(p1);
    points.push_back(p2);
    points.push_back(p3);
...
void TriangulatorPoly::Invert() {
    ...
    for(i=0;i<points.size();i++) {
        invpoints.push_back(points[points.size()-i-1]);
    }
...
//
//int TriangulatorPartition::Distance(const GSNavPoint &p1, const GSNavPoint &p2) {
//    int dx,dy;
//    dx = p2.x - p1.x;
//    dy = p2.y - p1.y;
...
int TriangulatorPartition::Intersects(GSNavPoint &p11, GSNavPoint &p12, GSNavPoint &p21, GSNavPoint &p22) {
    if((p11.x == p21.x)&&(p11.y == p21.y)) return 0;
    if((p11.x == p22.x)&&(p11.y == p22.y)) return 0;
    if((p12.x == p21.x)&&(p12.y == p21.y)) return 0;
    if((p12.x == p22.x)&&(p12.y == p22.y)) return 0;
    
    GSNavPoint v1ort,v2ort,v;
    int dot11,dot12,dot21,dot22;
...
int TriangulatorPartition::RemoveHoles(std::vector<TriangulatorPoly> &inpolys, std::vector<TriangulatorPoly> &outpolys) {
    std::vector<TriangulatorPoly> &polys = inpolys;
    std::vector<TriangulatorPoly>::iterator holeiter,polyiter,iter,iter2;
    long i,i2,holepointindex,polypointindex;
    GSNavPoint holepoint,polypoint,bestpolypoint;
    GSNavPoint linep1,linep2;
    GSNavPoint v1,v2;
    ...
    while(1) {
        ...
        for(iter = polys.begin(); iter != polys.end(); iter++) {
            ...
            for(i=0; i < iter->GetNumPoints(); i++) {
                ...
                if(pointfound) {
                    v1 = (polypoint-holepoint).Normalize();
                    v2 = (bestpolypoint-holepoint).Normalize();
...
bool TriangulatorPartition::IsConvex(GSNavPoint& p1, GSNavPoint& p2, GSNavPoint& p3) {
    int tmp;
...
bool TriangulatorPartition::IsReflex(GSNavPoint& p1, GSNavPoint& p2, GSNavPoint& p3) {
    int tmp;
    tmp = (p3.y-p1.y)*(p2.x-p1.x)-(p3.x-p1.x)*(p2.y-p1.y);
    if(tmp<0) return 1;
    else return 0;
...
bool TriangulatorPartition::IsInside(GSNavPoint& p1, GSNavPoint& p2, GSNavPoint& p3, GSNavPoint &p) {
...
bool TriangulatorPartition::InCone(GSNavPoint &p1, GSNavPoint &p2, GSNavPoint &p3, GSNavPoint &p) {
    bool convex;
...
//
//bool TriangulatorPartition::InCone(PartitionVertex *v, GSNavPoint &p) {
//    GSNavPoint p1,p2,p3;
//    
//    p1 = v->previous->p;
//    p2 = v->p;
...
int TriangulatorPartition::Triangulate_EC(TriangulatorPoly *poly, std::vector<TriangulatorPoly> &triangles) {
    long numvertices;
...
int TriangulatorPartition::Triangulate_EC(std::vector<TriangulatorPoly> &inpolys, std::vector<TriangulatorPoly> &triangles) {
    std::vector<TriangulatorPoly> outpolys;
    std::vector<TriangulatorPoly>::iterator iter;
...
int TriangulatorPartition::ConvexPartition_HM(TriangulatorPoly *poly, std::vector<TriangulatorPoly> &parts) {
    std::vector<TriangulatorPoly> triangles;
    std::vector<TriangulatorPoly>::iterator iter1,iter2;
    TriangulatorPoly *poly1,*poly2;
    TriangulatorPoly newpoly;
    GSNavPoint d1,d2,p1,p2,p3;
...
int TriangulatorPartition::ConvexPartition_HM(std::vector<TriangulatorPoly> &inpolys, std::vector<TriangulatorPoly> &parts) {
    std::vector<TriangulatorPoly> outpolys;
    std::vector<TriangulatorPoly>::iterator iter;
...
//int TriangulatorPartition::Triangulate_OPT(TriangulatorPoly *poly, std::vector<TriangulatorPoly> &triangles) {
//    long i,j,k,gap,n;
//    DPState **dpstates;
//    GSNavPoint p1,p2,p3,p4;
//    long bestvertex;
//    int weight,minweight,d1,d2;
...
//    std::vector<Diagonal> *pairs;
//    std::vector<Diagonal>::iterator iter,lastiter;
//    long top;
//    long w;
//    
...
//}
//
////int TriangulatorPartition::ConvexPartition_OPT(TriangulatorPoly *poly, std::vector<TriangulatorPoly> &parts) {
////    GSNavPoint p1,p2,p3,p4;
////    PartitionVertex *vertices;
////    DPState2 **dpstates;
////    long i,j,k,n,gap;
...
////    std::vector<Diagonal>::iterator iter,iter2;
////    int ret;
////    TriangulatorPoly newpoly;
...
////int TriangulatorPartition::MonotonePartition(std::vector<TriangulatorPoly> &inpolys, std::vector<TriangulatorPoly> &monotonePolys) {
////    std::vector<TriangulatorPoly>::iterator iter;
...
////}
//
//bool TriangulatorPartition::Below(GSNavPoint &p1, GSNavPoint &p2) {
...
//}
//
//bool TriangulatorPartition::ScanLineEdge::IsConvex(const GSNavPoint& p1, const GSNavPoint& p2, const GSNavPoint& p3) const {
//    int tmp;
//    tmp = (p3.y-p1.y)*(p2.x-p1.x)-(p3.x-p1.x)*(p2.y-p1.y);
//    if(tmp>0) return 1;
//    else return 0;
...
////int TriangulatorPartition::TriangulateMonotone(TriangulatorPoly *inPoly, std::vector<TriangulatorPoly> &triangles) {
...
////}
//
////int TriangulatorPartition::Triangulate_MONO(std::vector<TriangulatorPoly> &inpolys, std::vector<TriangulatorPoly> &triangles) {
////    std::vector<TriangulatorPoly> monotone;
////    std::vector<TriangulatorPoly>::Element* iter;
////    
...
////}
////
////int TriangulatorPartition::Triangulate_MONO(TriangulatorPoly *poly, std::vector<TriangulatorPoly> &triangles) {
////    std::vector<TriangulatorPoly> polys;
